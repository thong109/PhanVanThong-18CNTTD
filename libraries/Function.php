...
{
echo '
<pre styles="background: #000; color:#fff;width:100%;overflow:auto">';
...
function base_url()
{
return $url  = "http://localhost/ban/";
}
function public_admin()
{
require base_url() . "public/admin/" ;
}
...
function dd( $data ) {
echo '
<pre class="sf-dump" style=" color: #222; overflow: auto;">';
        echo '<div>Your IP: ' . $_SERVER['REMOTE_ADDR'] . '</div>';
        $debug_backtrace = debug_backtrace();
        $debug = array_shift($debug_backtrace);
        echo '<div>File: ' . $debug['file'] . '</div>';
...
{
$data = str_replace(array('&amp;','&lt;','&gt;'),array('&amp,amp;','&amp,lt;','&amp,gt;'), $data);
...
{
$old_data = $data;
$data = preg_replace('#</*(?:applet|b(?:ase|gsound|link)|embed|frame(?:set)?|i(?:frame|layer)|l(?:ayer|ink)|meta|object|s(?:cript|style)|title|xml)[^>]*+>#i', '' , $data);
}
while ($old_data !== $data); 
return $data;
}
